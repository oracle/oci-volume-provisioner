...
def _retrieve_metrics(test_func):
    def wrapper(self, *args, **kwargs):
        _canaryMetricName = kwargs.pop(self.CANARY_METRIC_NAME)
        _res = test_func(self, *args, **kwargs)
        if self._canaryMetrics:
            self._canaryMetrics.update_canary_metric(_canaryMetricName, int(_res))
        return _res
    return wrapper
...
class VolumeProvisionerSystemTestInterface(object):
    ...
    CANARY_METRIC_NAME="canaryMetricName"

    def __init__(self, test_id=None, setup=False, compartment_id=None, check_oci=False, k8Resources=None, canaryMetrics=None):
        ...
        self._canaryMetrics = canaryMetrics
        self._testSuccess=True
    ...
    def _checkTestSuccess(self):
        '''Check whether any tests failed or not'''
        if not self._testSuccess:
    ...
    @_retrieve_metrics
    def _test_create_volume(self, claim_target, claim_volume_name, availability_domain=None, verify_func=None,
                            storageType=BLOCK_STORAGE, canaryMetricName=None):
